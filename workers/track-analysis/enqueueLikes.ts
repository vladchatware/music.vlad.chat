...
if (!userId) throw new Error("SOUNDCLOUD_USER_ID is required");

const queue = new AnalysisQueueClient(getConvexSiteUrl(), secret);
const likes = await allLikes(userId);
...
for (let offset = 0; offset < eligible.length; offset += 20) {
  const result = await queue.enqueue(eligible.slice(offset, offset + 20).map((track) => track.id));
