...
function makeTimbralMeterTrack(
  durationSec: number,
  bpm: number,
  ...
  for (let beat = 0; beat * beatSamples < signal.length; beat += 1) {
    const start = beat * beatSamples;
    const frequency = beat % 4 === downbeatPhase ? 75 : 1_200;
    for (let index = 0; index < 1_600 && start + index < signal.length; index += 1) {
      signal[start + index] += Math.exp(-index / 360) *
        Math.sin((2 * Math.PI * frequency * index) / ANALYSIS_SAMPLE_RATE);
    }
  }
  return signal;
}

function makeUnaccentedClickTrack(durationSec: number, bpm: number): Float32Array {
  const signal = new Float32Array(durationSec * ANALYSIS_SAMPLE_RATE);
  const beatSamples = Math.round((60 / bpm) * ANALYSIS_SAMPLE_RATE);
  for (let beat = 0; beat * beatSamples < signal.length; beat += 1) {
    const start = beat * beatSamples;
    for (let index = 0; index < 800 && start + index < signal.length; index += 1) {
      signal[start + index] += Math.exp(-index / 180) *
        Math.sin((2 * Math.PI * 120 * index) / ANALYSIS_SAMPLE_RATE);
    }
  }
  return signal;
}
...
describe("track analyzer", () => {
  ...
  it("uses low-frequency accents when every beat has equal broadband energy", () => {
    const signal = makeTimbralMeterTrack(24, 120, 2);
    const beats = Array.from({ length: 48 }, (_, index) => index * 0.5);
    const result = estimateDownbeats(signal, beats);
    expect(result.firstDownbeatSec).toBe(1);
    expect(result.downbeatsSec.slice(0, 3)).toEqual([1, 3, 5]);
    expect(result.confidence).toBeGreaterThan(0.5);
  });

  it("keeps confidence low when four-beat phase is genuinely ambiguous", () => {
    const signal = makeUnaccentedClickTrack(24, 120);
    const beats = Array.from({ length: 48 }, (_, index) => index * 0.5);
    expect(estimateDownbeats(signal, beats).confidence).toBeLessThan(0.1);
  });
